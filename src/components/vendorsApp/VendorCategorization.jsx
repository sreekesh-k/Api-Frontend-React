function VendorCategorization(props) {
    ...
    const [gtScore, setGtScore] = React.useState(0);
    const [loading, setLoading] = React.useState(false);
    const [loadingScores, setLoadingScores] = React.useState(false);
    const [scoreRatingGroups, setScoreRatingGroups] = React.useState([])
    const [scoreRating, setScoreRating] = React.useState("");
    const [saveArray, setSaveArray] = React.useState([]);
    const [_saveData, set_SaveData] = React.useState([]);

    const dispatch = ReactRedux.useDispatch();

    const { vendorType, dataa, isLoading, vendorId, isInViewMode, isCentrilized, ReviewerRemarks, LastReviewSentOn, Reviewers } = ReactRedux.useSelector((state) => {
        return {
            vendorType: state.vendor.vendorType, dataa: state.vendor.categorization.data,
            isLoading: state.vendor.categorization.loading, gtScore: state.vendor.categorization.gtScore,
            scoreRating: state.vendor.categorization.gtScore, vendorId: state.vendor.vendorId,
            isInViewMode: state.vendor.editAccess.isInViewMode, vendorId: state.vendor.vendorId,
            isCentrilized: state.vendor.isUserCentrilized, ReviewerRemarks: state.vendor.categorization.notificationData.ReviewerRemarks,
            LastReviewSentOn: state.vendor.categorization.notificationData.LastReviewSentOn, Reviewers: state.vendor.categorization.notificationData.Reviewers,
        };
    })
    ...
    const handleDataChange = (e, ScoreModels, parameterId) => {
        ...
        _saveArray.forEach((obj) => {
            newObj = Object.keys(obj).filter(objKey =>
                objKey !== 'paramId').reduce((newObj, key) => {
                    newObj[key] = obj[key];
                    return newObj;
                }, {}
                );
        ...
        dispatch({
            type: "UPDATE_SAVING_CATEGORIZATION_DATA",
            payload: arr
        })
        // --------------
    ...
    const alphabetsArr = Array.from({ length: 26 }, (_, i) => String.fromCharCode(97 + i));
    ...
    const areFieldsDisabled = (vendorType == "DSA" || isInViewMode || !isCentrilized)

    const btnWrapper = document.querySelector(".historyBtn-wrapper");

    return <div className="categorization-wrapper vd-access-outer">
       
        {
            vendorType === "DSA" && <div className="categorization-overlay" >
                <p className="c-overlay-text">Your are not allowed to make changes in this Tab</p>
            </div>
        }
        ...
        </Row>
        {
            dataa.map((param, key) => {
                ...
                return <Row key={key} className="vendor-data-rows" gutter={20}>
                    <Col span="7">
                        <p className="vendor-parameter-name">{`${key + 1}. ${param.ScoringTitle}`}</p>
                    </Col>
                    <Col span="5" className="vc-row-item">
                        {
                            param.SubParams === null && <Select
                                ...
                                size="small"
                                defaultValue={param.ScoreModels.filter((s) => s.IsSelected).map((v) => v.value)}
                                style={{ width: 250 }}
                    ...
                    <Col span="6">
                        {
                            param.SubParams === null && <TextArea type="text"
                                onChange={(e) => handleDataChange(e, param.ScoreModels, param.Id)}
                                className="data-input"
                                value={
                                    param.ScoreModels.filter((s) => s.IsSelected).map((v) => v.Data)
                                }
                                disabled={areFieldsDisabled || param.ScoreModels.filter((s) => s.IsSelected).lenght < 1}
                    ...
                    <Col span="2" className="s-model-border-right vc-row-item">
                        {
                            param.SubParams === null && <Input type="text" disabled className="s-model-input"
                                value={
                                    param.ScoreModels.filter((s) => s.IsSelected).map((v) => v.Score)
                                }
                                defaultValue="0"
                            />
                        }
                    ...
                    <Col span="2" className="s-model-border-right vc-row-item">
                        {
                            param.SubParams === null && <Input type="text" disabled className="s-model-input"
                                value={
                                    param.ScoreModels.filter((s) => s.IsSelected).map((v) => v.Weights)
                                }
                                defaultValue="0"
                            />
                        }
                    ...
                    <Col span="2" className="s-model-border-right vc-row-item">
                        {
                            param.SubParams === null && <Input type="text" disabled className="s-model-input"
                                value={
                                    param.ScoreModels.filter((s) => s.IsSelected).map((v) => v.Rating)
                                }
                                defaultValue="0"
                            />
                        }
                    ...
                    {
                        param.SubParams !== null ? param.SubParams.map((sp, key) => {
                            return <Col span="24" key={key}>
                                <Row className="scoring-model" gutter={20}>
                                    ...
                                    <Col span="5" className="vc-row-item">
                                        {
                                            <Select
                                                ...
                                                className="vendor-select"
                                                size="small"
                                                defaultValue={sp.ScoreModels.filter((s) => s.IsSelected).map((v) => v.value)}
                                                style={{ width: 250 }}
                                                options={sp.ScoreModels}
                                                onChange={(e) => handleChange2(e, param.Id, sp.Id)}
                                                allowClear={true}
                                                style={{ marginBottom: ".5vh" }}                                                
                                            />
                                        }
                                    ...
                                    <Col span="6">
                                        {
                                            <TextArea type="text"
                                                className="data-input"
                                                value={
                                                    sp.ScoreModels.filter((s) => s.IsSelected).map((v) => v.Data)
                                                }
                                                onChange={(e) => handleDataChange2(e, sp.ScoreModels, param.Id, sp.Id)}
                                                disabled={areFieldsDisabled || sp.ScoreModels.filter((s) => !s.IsSelected).length < 1}
                                                autoSize={{ minRows: 2, maxRows: 2 }}
                                            />
                                        }
                                    </Col>
                                    <Col span="2" className="s-model-border-right vc-row-item">
                                        {
                                            <Input type="text" disabled className="s-model-input"
                                                value={
                                                    sp.ScoreModels.filter((s) => s.IsSelected).map((v) => v.Score)
                                                }
                                                defaultValue=""
                                            />
                                        }
                                    ...
                                    <Col span="2" className="s-model-border-right vc-row-item">
                                        {
                                            <Input type="text" disabled className="s-model-input"
                                                value={
                                                    sp.ScoreModels.filter((s) => s.IsSelected).map((v) => v.Weights)
                                                }
                                                defaultValue=""
                                            />
                                        }
                                    ...
                                    <Col span="2" className="s-model-border-right vc-row-item">
                                        {
                                            <Input type="text" disabled className="s-model-input"
                                                value={
                                                    sp.ScoreModels.filter((s) => s.IsSelected).map((v) => v.Rating)
                                                }
                                                defaultValue=""
                                            />
                                        }
        ...
        <div className="vd-reviewer-wrapper" style={{ marginTop: "3vh" }}>
            ...
            <div className="vc-reviewer-top">
                ...
                <div className="vc-top-right">
                    ...
                    <p>Send for Review</p>
                        <Button className="vd-date-button" type="primary" onClick={handleSaveReviewers} disabled={areFieldsDisabled}>Send</Button>
                    </div>
                    <div className="vc-last-review-outer">
                        <p>Last review sent on</p>
                        <Input value={moment(LastReviewSentOn).format('DD-MM-YYYY, h:mm')} disabled={true} style={{height: "5vh"} } />
                    </div>
                </div>
            </div>
